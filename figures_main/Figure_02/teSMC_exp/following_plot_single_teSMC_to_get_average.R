...
tpop1 <- (rdf_total$t_pop %>% unique() %>% sort())[1]
tpop2 <- (rdf_total$t_pop %>% unique() %>% sort())[2]

complete_df <- rdf_total %>%
  subset(t_sigma %in% c(tsigma1, tsigma2))
...

{
  ...
  d1 <- stat_df %>%
    ...
    geom_step(
      data = data.frame(
        t = c(min(rdf$t[rdf$t >= mlimits[1]]), tpop, Inf) + 1,
        mean_pop_size = c(pop_1, pop_2, pop_2),
        inference_model = as.factor("true"),
        file = as.factor("true"),
        quant025_pop_size = NA,
        quant975_pop_size = NA
      ),
      col = "black",
      size = 1
    ) +
...
filename <- paste0(c("examplatory_inferred_dem_sigma_fun_with_95conf.", tpop, ".pdf"), collapse = "")
pdf(filename)
show(p)
dev.off()
...
filename <- paste0(c("examplatory_inferred_dem_sigma_fun_with_95conf.", tpop, "_alt.pdf"), collapse = "")
pdf(filename)
show(p_alt)
dev.off()
