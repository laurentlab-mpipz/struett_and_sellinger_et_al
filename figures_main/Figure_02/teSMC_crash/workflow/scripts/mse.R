...
mat_save_p=matrix(NA,nrow=(1*1*nsim),ncol=40)
mat_save_t=matrix(NA,nrow=(1*1*nsim),ncol=40)
mat_save_s=matrix(NA,nrow=(1*1*nsim),ncol=40)
count_p=0
count_t=0
count_s=0
...
for(x_rep in 1:nsim){
    ...
    demography = matrix(snakemake@config$population_sizes_backward_in_time,
                          byrow = T, ncol = 4)[which_demography,]

    N_rec = demography[1]
    N_anc = demography[3]
    t_pop = demography[4]

    pop_size_t = rep(N_rec,length(x_time))
    if (any(x_time>t_pop)) {
      pop_size_t[which(x_time>t_pop)]=N_anc
    }
