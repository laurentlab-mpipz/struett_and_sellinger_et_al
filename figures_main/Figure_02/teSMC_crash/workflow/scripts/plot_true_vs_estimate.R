...
# read estimated demography
result = readRDS(snakemake@input[["data"]])
...
N = accomodate_times_to_estimated_times(N, df$Tc, mu_estim/real_mu)
sigma = accomodate_times_to_estimated_times(sigma, df$Tc, mu_estim/real_mu)

## plot N, r, sigma over time
p1 <- ggplot()+
  geom_step(data = df,
            aes(x=Tc*(mu_estim/real_mu),y=Xi*0.5*mu_estim/real_mu),
            col=mcol[2])+
  geom_point(data = df,
            aes(x=Tc*(mu_estim/real_mu),y=Xi*0.5*mu_estim/real_mu),
            col="black", size = 1.05)+
...
p2 <- ggplot()+
  geom_step(data = df,
            aes(x=Tc*(mu_estim/real_mu),y=sigma),
            col=mcol[2])+
  geom_point(data = df,
            aes(x=Tc*(mu_estim/real_mu),y=sigma),
            col="black", size = 1.05)+
...
p3 <- ggplot()+
  geom_step(data = df,
            aes(x=Tc*(mu_estim/real_mu),y=rho),
            col=mcol[2])+
  geom_point(data = df,
            aes(x=Tc*(mu_estim/real_mu),y=rho),
            col="black", size = 1.05)+
