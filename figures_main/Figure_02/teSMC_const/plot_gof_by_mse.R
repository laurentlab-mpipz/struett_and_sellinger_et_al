...
# add which tpop it happens
pop_config <- read_yaml("config/config.yaml")$population_sizes_backward_in_time
rmse_csv$tpop <- sapply(rmse_csv$demography, function(demography_index) {
  tpop_string <- pop_config[[demography_index+1]][[2]][[2]]
  tpop_string <- str_replace(tpop_string,"[^0-9.]","" )
  return(as.numeric(tpop_string))
})
