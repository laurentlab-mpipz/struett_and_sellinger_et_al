...
"""
snake to generate the pods table including parameters and summary statistics
"""

import msprime
import warnings

rule simulate_summary_stat_table_for_pods:
    output: "data/pod_{pod_iloc}/params_and_sumstats_{ident}_.gzip"
    input:
        rand_table = "data/rands/rands_pod_{pod_iloc}_.gzip",
    ...
    run:
        def main(identifier_of_simulation):
            ...
            for independent_region_no in range(params.configuration["num_independent_regions"]):
                ...
                df_prior_parameter = tsabc.vector_to_DataFrame(vector_prior_parameter, 'param', ident)
        ...
        df = pd.concat([main(my_ident) for my_ident in batch_dict_pods[float(wildcards.ident)]])

        # save as binary
        df.to_pickle(str(output))

rule aggregate_single_pod_data:
    output: "data/table/params_and_sumstats_pod_{pod_iloc}.table.gzip"
    input:
        simstats = expand("data/pod_{{pod_iloc}}/params_and_sumstats_{ident}_.gzip",
