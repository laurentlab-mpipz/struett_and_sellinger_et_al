...
class tsabc2:
    ...
    def draw_integer(self, my_lower, my_upper, my_distribution_type):
        ...
        if my_lower == my_upper:
            return my_lower
        elif my_distribution_type == "uniform":
            return np.random.randint(my_lower, my_upper + 1)
        elif my_distribution_type == "exponential":
            sys.exit(
                """exponential (geometric) distributions are not implemented, yet"""
            )
        elif my_distribution_type == "logunif":
            return int(loguniform.rvs(my_lower, my_upper, size=1)[0])
        else:
            sys.exit("""only accepting 'uniform', 'exponential', 'logunif' as distribution type
                \rtechnically an exponential integer refers to a geometric distribution""")
    ...
    def draw_float(self, my_lower, my_upper, my_distribution_type):
        ...
        if my_lower == my_upper:
            ...
            sys.exit("did not define exponential yet")
        elif my_distribution_type == "logunif":
            return loguniform.rvs(my_lower, my_upper, size=1)[0]
        else:
            sys.exit(
                """only accepting 'uniform', 'exponential', 'logunif' as distribution type
                    \rtechnically an exponential integer refers to a geometric distribution
                """
            )
    ...
    def draw_parameter_from_prior_distribution(self, prior_distribution_definition):
        ...
        assert (
            len(prior_distribution_definition) == 4
        ), "need a tuple containting 4 values: lower, upper, valuetype, distributiontype"

        if prior_distribution_definition[2] == "int":
            return self.draw_integer(
                *prior_distribution_definition[0:2], prior_distribution_definition[3]
            )
            ...
            sys.exit("only accepting 'int' or 'float' as value type")

    def run_transition_to_selfing_model(
        self, p_s_r, t_p, p_s_a, s_r, t_s, s_a, r, nsample, L, n_dtwf, seed
    ):
        """building the chronology of events and apply to msprime"""
        # transform the model into a time-row matrix
        tables = np.array(
            self.get_time_sequence_for_transition_to_selfing_model(
        ...
        demography_general = []
        demography_general.append(msprime.SimulationModelChange(n_dtwf, "smc_prime"))

        # run the first time segment
        events_phase_0 = [
            event for event in demography_general if event.time < matrix[1, 0]
        ]
        ...
        for i in range(1, np.size(matrix, 0)):
            ...
            if i < np.size(matrix, 0) - 1:
                end_time = matrix[i + 1, 0]
            else:
                end_time = float("inf")

            # event filtering
            events_of_this_phase = [
                event
                for event in demography_general
    ...
    def get_time_sequence_for_transition_to_selfing_model(
        self,
        population_size_recent,
        t_population_size_change,
    ...
    def get_N_r_over_time(self, times: list, pop_sizes: list, sigmas: list, r: list):
        ...
        """
        lam = lambda l: all(
            [True if len(l[i - 1]) == len(l[i]) else False for i in range(1, len(l))]
        )
        ...
        return np.column_stack(
            (
                times,
                [
                    self.get_N_r_given_sigma(pop_sizes[i], r[i], sigmas[i])
    ...
    def get_binned_ld_from_ts(
        self, ts, num_max_random_mutations, chrom_length, breaks, seed
    ):
    ...
    def get_ld(self, ts_mutated, num_sites, seed):
        assert 'SLiM' in ts_mutated.metadata, "this function is optimized for slim tree seqs, you should probably use the _get_ld function"

        tables = ts_mutated.dump_tables()
        tables.mutations.clear()
        no_mut = tables.tree_sequence()
        no_mut = no_mut.simplify(reduce_to_site_topology=True)

        # some low estimate for the mutating
        low_mut_rate = ts_mutated.diversity()/(4 * ts_mutated.sequence_length)

        ts = no_mut
        print("\rmuts:", ts.num_sites, end="\t")
        while (ts.num_sites < 2000):
            ts = msprime.sim_mutations(ts, rate=low_mut_rate, discrete_genome=False)
            print("\rmuts:", ts.num_sites, end="\t")
        print()
    ...
    def _get_ld(self, ts_mutated, num_sites, seed):
        ...
        [
            tables.mutations.add_row(
                old_mutations[i].site,
                old_mutations[i].node,
                old_mutations[i].derived_state,
                old_mutations[i].parent,
                old_mutations[i].metadata,
                old_mutations[i].time,
            )
            for i in m_ids_sampled
        ]
        ...
        [
            tables.sites.add_row(
                old_sites[i].position,
                old_sites[i].ancestral_state,
                old_sites[i].metadata,
            )
            for i in m_ids_sampled
        ]
    ...
    def get_breaks_for_binnning_tl_true(
        self, tl_true_binning_scale_factor, max_population_size, num_bins
    ):
        """get the breaks, lower bound 0, upper bound inf returns a np.array with the breaks"""
        breaks = np.array(
            [
                tl_true_binning_scale_factor
                * max_population_size
                * (-1)
                * np.log(1 - i / (num_bins))
    ...
    def get_list_tmrcas_and_lengths(self, ts, num_pairs_max):
        ...
        pair_ts_list = self.get_list_of_tupled_ts(
            ts, num_pairs_max, 2, np.random.randint(0, 4294967296)
        )
        tl_table = [
            [
                [tree.interval[1] - tree.interval[0], tree.time(tree.root)]
                if len(tree.roots) == 1
    ...
    def get_list_of_tupled_ts(self, ts, num_pairs_max, tuple_size, seed):
        ...
        return [ts.simplify(samples=pair) for pair in my_pairs]
    ...
    def sample_from_iterable(self, iterable, samplesize, seed):
        ...
        return (
            x
            for _, x in nlargest(
                samplesize, ((np.random.uniform(), x) for x in iterable)
            )
        )

    def get_binned_tmrcas_from_true_tl_array(
        self, tl_true_arr, breaks_age, chrom_length, num_pairs, min_len_break
    ):
        ...
        breaks_len = 10 ** np.arange(
            0,
            np.log10(chrom_length),
            (np.log10(chrom_length)) / (len(breaks_age) - 1),
        )[1:]
        breaks_len = np.sort(
            np.append([0, np.inf], breaks_len[breaks_len > min_len_break])
        )

        bins = (
            np.histogram2d(LEN, AGE, [breaks_len, breaks_age], density=False)[0]
            / num_pairs
        )
        ...
        if False:
            ...
            print(
                f"tl_true_vector non-zeros:\t{round(100*(vector_tl_true_binned != 0).sum()/len(vector_tl_true_binned), 2)}%"
            )
    ...
    def discretize_list_tmrca_lists(self, tmrcas, breaks):
        ...
        return [
            pd.cut(
                tmrca,
                breaks,
                labels=[i for i in range(len(breaks) - 1)],
                include_lowest=True,
            )
            for tmrca in tmrcas
        ]
    ...
    def transition_matrices_from_tiscretized_tmrcas(self, dtmrcas, nstates):
        ...
        return np.array(
            [self.get_transition_matrix(dtmrca, nstates) for dtmrca in dtmrcas]
        )
    ...
    def get_list_of_pairwise_diversities(
        self, ts, num_pairs_max, stat_mode, window, tuple_size, window_step, seed
    ):
        ...
        if window - window_step > 0:
            ...
            if False:
                print("window_step equals window_size")
    ...
    def get_breaks_for_binning_tm_win(
        self, discretizing_scale, window_pi_size, mutation_rate, max_pop_size, num_bins
    ):
        """calculate the breakpoints to discretize the diversity for
        calculating the transition matrix of a sliding window pi"""
        prelim_breaks = np.array(
            [
                -1
                * discretizing_scale
                * window_pi_size
    ...
    def vector_to_DataFrame(self, my_vector, my_vectors_prefix, index):
        ...
        return pd.DataFrame(
            data=[my_vector],
            columns=[f"{my_vectors_prefix}_{i}" for i in range(len(my_vector))],
            index=[index],
        )
