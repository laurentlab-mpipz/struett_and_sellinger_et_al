...
build_HMM_matrix<-function(n=20,rho,beta=1,L,Pop=T,Xi=NA,Beta=1,scale=c(1,0),sigma=0,Sigma=0,Big_Window=F,npair=2,FS=F,cut_edge=F){
  if(cut_edge){
    n=n+1
  }
...
Baum_Welch_algo_t<-function(Os, maxIt =20,L,mu,theta_W,Rho,beta=1,Popfix=T,SB=F,k=20,BoxB=c(0.1,1),BoxP=c(3,3),Boxr=c(1,1),maxBit=1,pop_vect=NA,window_scaling=c(1,0),sigma=0.00,SF=F,Boxs=c(0,0.97),ER=F,BW=F,NC=1,redo_R=F,mu_b=1,SCALED=F,Big_Window=F,EM=T,FS=F){
                ...
                if(SF){
                  ...
                  if(!Popfix){
                    ...
                    function_to_minimize<-function(param){
                      ...
                      builder=build_HMM_matrix_t(n,(rho_),beta=beta,Pop = Pop,Xi=Xi,L=L,Beta=Beta,scale=window_scaling,sigma = sigma_t,Sigma = Self,FS=FS,Big_Window=Big_Window,npair=npair)
                      Q=builder[[1]]
                      Q=t(Q)
                      A=as.vector(Q)
                      keep=which(A>0&as.vector(Big_Xi)>0)
  ...
  res$N = N # estimated transition matrix
  return(res)
}
...
Optimize_N_t<-function(file,Rho,theta=NA,L,n=30,ER=T,Pop=T,SB=FALSE,SF=FALSE,BoxB=c(0.1,1),BoxP=c(3,3),Boxr=c(1,1),Boxs=c(0,0.97),pop_vect=NA,window_scaling=c(1,0),sigma=0,beta=1,SCALED=F,Big_Window=F,NC=1,npair=2,FS=F,BW=F,mut=F,Correct_window=F){
  cut_edge=F
  mu=theta/(2*L)
  ...
  if(SF){
    ...
    sigma=max(sigma,Boxs[1])
    Self=sigma
    oldsigma=rep(sigma,length(pop_vect))
  }
  if(Pop){
    oldXi_=rep((BoxP[1]/sum(BoxP)),Klink)
    oldXi=vector()
